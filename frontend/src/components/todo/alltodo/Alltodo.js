...
export default function Alltodo() {
  ...
  const [tasks, settasks] = useState([
    {
      ...
      describe:
        "This is a demo task that you can see and there are lote that need to be improve",
      project: "Home",
      like: "false",
  ...
  useEffect(() => {
    BaseService.getResponse("gettasks", reactLocalStorage.get("token")).then(
      (resp) => {
        settasks(resp.data);
      }
    );
              ...
              <Box boxShadow={2} bgcolor="background.paper">
                <ListItem key={index} button>
                  ...
                  <ListItemSecondaryAction>
                    ...
                    >
                      <ListItemText
                        primary={formatDate(task.createdAt)}
                ...
                <Collapse in={checked.indexOf(index) !== -1} id={index}>
                  <List disablePadding>
                    <ListItem className={classes.nested}>
                      <ListItemText
                        primary={`Assign by     ${task.assignby}`}
                      />
                      <Button
                        variant="outlined"
                        size="small"
